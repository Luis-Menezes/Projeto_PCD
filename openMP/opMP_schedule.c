...
static double assignment_step_1d(const double *X, const double *C, int *assign, int N, int K, 
    ...
    // Escolhe a estratégia de schedule baseada no parâmetro
    if(strcmp(schedule_type, "static") == 0) {
        if(chunk_size > 0) {
            #pragma omp parallel for schedule(static, chunk_size) reduction(+:sse)
            for(int i=0; i<N; i++){
                int best = -1;
                double bestd = 1e300;
                for(int c=0; c<K; c++){
        ...
        if(chunk_size > 0) {
            #pragma omp parallel for schedule(dynamic, chunk_size) reduction(+:sse)
            for(int i=0; i<N; i++){
                int best = -1;
                double bestd = 1e300;
                for(int c=0; c<K; c++){
        ...
        if(chunk_size > 0) {
            #pragma omp parallel for schedule(guided, chunk_size) reduction(+:sse)
            for(int i=0; i<N; i++){
                int best = -1;
                double bestd = 1e300;
                for(int c=0; c<K; c++){
    ...
    else { // default ou "auto"
        #pragma omp parallel for schedule(auto) reduction(+:sse)
        for(int i=0; i<N; i++){
            int best = -1;
            double bestd = 1e300;
            for(int c=0; c<K; c++){
...
static double calculaSilhouette(const double *X, const double *C, const int *assign, 
                                int N, int K, const char* schedule_type, int chunk_size){
    double silhouette_sum = 0.0;
    
    // Escolhe a estratégia de schedule baseada no parâmetro
    if(strcmp(schedule_type, "static") == 0) {
        if(chunk_size > 0) {
            #pragma omp parallel for schedule(static, chunk_size) reduction(+:silhouette_sum)
            for(int i=0; i<N; i++){
                silhouette_sum += silhouetteSample(X, C, assign, i, N, K);
            }
        } else {
            #pragma omp parallel for schedule(static) reduction(+:silhouette_sum)
            for(int i=0; i<N; i++){
                silhouette_sum += silhouetteSample(X, C, assign, i, N, K);
            }
        }
        ...
        if(chunk_size > 0) {
            #pragma omp parallel for schedule(dynamic, chunk_size) reduction(+:silhouette_sum)
            for(int i=0; i<N; i++){
                silhouette_sum += silhouetteSample(X, C, assign, i, N, K);
            }
        } else {
            #pragma omp parallel for schedule(dynamic) reduction(+:silhouette_sum)
            for(int i=0; i<N; i++){
                silhouette_sum += silhouetteSample(X, C, assign, i, N, K);
            }
        }
        ...
        if(chunk_size > 0) {
            #pragma omp parallel for schedule(guided, chunk_size) reduction(+:silhouette_sum)
            for(int i=0; i<N; i++){
                silhouette_sum += silhouetteSample(X, C, assign, i, N, K);
            }
        } else {
            #pragma omp parallel for schedule(guided) reduction(+:silhouette_sum)
            for(int i=0; i<N; i++){
                silhouette_sum += silhouetteSample(X, C, assign, i, N, K);
            }
        }
        ...
        #pragma omp parallel for schedule(auto) reduction(+:silhouette_sum)
        for(int i=0; i<N; i++){
            silhouette_sum += silhouetteSample(X, C, assign, i, N, K);
        }
    }
...
static void kmeans_1d(const double *X, double *C, int *assign,
                      ...
                      int *iters_out, double *sse_out, const char* schedule_type, int chunk_size)
{
    double prev_sse = 1e300;
    double sse = 0.0;
    int it;
    for(it=0; it<max_iter; it++){
        sse = assignment_step_1d(X, C, assign, N, K, schedule_type, chunk_size);
...
int main(int argc, char **argv){
    ...
    const char *schedule_type = (argc>6)? argv[6] : "static";
